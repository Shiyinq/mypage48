...
def format_member(member_id: str, member_name: str, member_url: str, ref_member: Optional[Dict[str, Any]]) -> Dict[str, Any]:
...
def merge_data():
    ...
    # 4. Save output
    output_events = os.path.join(historical_dir, f'events.schedule_{min_year}_to_{max_year}.json')
    with open(output_events, 'w', encoding='utf-8') as f:
        json.dump(all_events, f, indent=2, ensure_ascii=False)
    print(f"Saved events to {output_events}")
    
    output_members = os.path.join(historical_dir, f'members_{min_year}_to_{max_year}.json')
