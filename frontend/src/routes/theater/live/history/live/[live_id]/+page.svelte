<script lang="ts">
	...
	$effect(() => {
		if (liveId) {
			// Untrack the promise to avoid effect tracking issues, but liveId is tracked
	...
	// API now returns absolute URLs for screenshots and image
</script>

<SEO title={replayData?.member_name || 'Live Detail'} path={$page.url.pathname} />
				...
				>
					...
					>
						...
						<div class="flex-1 w-full flex flex-col sm:flex-row gap-5 sm:gap-6 min-w-0">
							<!-- Left Column: Siaran & Statistik Stacked -->
							<div class="shrink-0 flex flex-col gap-6 w-full lg:w-[320px]">
								<!-- Compact Waktu Siaran -->
