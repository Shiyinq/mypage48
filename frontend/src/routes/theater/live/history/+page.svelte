<script lang="ts">
	...
	import { Tv, History, Clock, ChevronLeft, Calendar } from 'lucide-svelte';
	...
	<div class="flex-1 overflow-y-auto" onscroll={(e) => (scrollY = e.currentTarget.scrollTop)}>
		<div class="max-w-7xl mx-auto px-4 sm:px-6 pt-6 pb-32 relative z-10">
			...
			<div class="flex items-center gap-4 mb-8">
				<div
					class="w-16 h-16 rounded-2xl bg-white dark:bg-zinc-900 shadow-xl border border-gray-100 dark:border-zinc-800 flex items-center justify-center shrink-0 -rotate-6"
				>
					<History size={32} class="text-red-500" />
				</div>
				<div>
			...
			{:else}
				<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
					{#each list as item (item._id)}
						<div
							class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 flex flex-col gap-3 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group"
						>
