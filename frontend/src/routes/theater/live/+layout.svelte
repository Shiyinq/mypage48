<script lang="ts">
	...
	const navItems = $derived([
		{ label: 'LIVE', mobileLabel: 'LIVE', href: '/theater/live', exact: true, icon: Tv },
		{
			label: t('theater.live.switchMultiview') || 'Multi-View',
			mobileLabel: 'Multi',
			href: '/theater/live/multiview',
			exact: true,
			icon: LayoutGrid
		},
		{
			...
			match: (path: string) =>
				path === '/theater/live/history' || path.startsWith('/theater/live/history/members'),
			icon: Globe
		},
		{
			...
			match: (path: string) => path.startsWith('/theater/live/history/watched'),
			icon: History
		},
		{
			label: t('liveHistory.pcLive.title') || 'PC Live',
			mobileLabel: 'PC Live',
			href: '/theater/live/pc',
			activeHref: '/theater/live/pc',
			exact: false,
			icon: ImageIcon
		}
	]);
