...
<div class="space-y-16 pt-4 md:pt-6 pb-12">
	...
	<div class="space-y-8">
		...
		{:else if birthdays.length === 0}
			<div class="p-12 rounded-[2.5rem] bg-slate-50 dark:bg-zinc-900/50 border-2 border-dashed border-slate-200 dark:border-zinc-800 text-center text-slate-400 font-bold uppercase tracking-widest text-sm">
			...
			<div class="flex gap-6 overflow-x-auto pb-6 scrollbar-hide snap-x snap-mandatory">
				{#each birthdays as member}
					<div class="flex-none w-44 snap-start">
						<div class="relative group aspect-[3/4] rounded-xl overflow-hidden bg-white dark:bg-zinc-900 shadow-sm hover:shadow-md transition-all duration-300">
							{#if member.img}
								<img
							...
							<div class="absolute top-4 left-0 right-0 flex justify-center z-30">
								<span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl backdrop-blur-md border border-white/20 {member.days_until === 0 ? 'bg-red-600 text-white animate-pulse' : 'bg-black/60 text-white'}">
									{getBirthdayText(member.days_until, $t)}
								</span>
							</div>

							<div class="absolute bottom-6 left-5 right-5 z-30 text-left">
								<div class="text-white font-black text-sm leading-tight mb-1 truncate drop-shadow-md">
									{member.name}
								</div>
			...
			{#each $upcomingEvents as event (event.id)}
				...
				>
					{#if isToday(event.date)}
						<div class="absolute inset-0 rounded-[2rem] animate-pulse pointer-events-none shadow-[0_0_30px_rgba(239,68,68,0.4)] z-0"></div>
					{/if}

					<!-- Content Container -->
					<div class="relative z-10 flex flex-row sm:block w-full h-full overflow-hidden rounded-[2rem] bg-white dark:bg-zinc-900">
						<!-- Image / Placeholder -->
						<div class="relative w-[40%] sm:w-full sm:h-full shrink-0 overflow-hidden bg-slate-50 dark:bg-zinc-800">
							{#if event.imageUrl}
								<img
									src={event.imageUrl}
									alt={event.title}
									class="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110"
								/>
								<div class="absolute inset-0 sm:hidden bg-gradient-to-r from-black/10 via-transparent to-black/5"></div>
								<div class="absolute inset-0 hidden sm:block bg-gradient-to-t from-black/95 via-black/40 to-transparent"></div>
							{:else}
								<div class="absolute inset-0 bg-gradient-to-br from-red-600 to-red-800 flex items-center justify-center opacity-20">
							...
							{#if isToday(event.date)}
								<div class="absolute top-4 left-4 z-20">
									<span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black text-white bg-red-600 shadow-xl shadow-red-500/30 ring-2 ring-white/20">
						...
						<!-- Details -->
						<div class="relative flex-1 p-5 flex flex-col justify-between sm:justify-end sm:absolute sm:inset-0 z-10">
							<!-- Types/Labels -->
							<div class="flex items-center gap-2 mb-2">
								{#if event.label}
									<div class="px-2 py-0.5 text-[9px] font-black rounded-md uppercase tracking-wider {event.label === 'JKT48' ? 'bg-red-600 text-white' : 'bg-zinc-800 text-white'}">
										{event.label}
									</div>
								{/if}
								{#if event.type}
									<div class="px-2 py-0.5 text-[9px] font-black rounded-md uppercase tracking-wider {event.type === 'EVENT' ? 'bg-pink-600 text-white' : event.type === 'SHOW' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-white'}">
										{event.type}
									</div>
								{/if}
							</div>
							...
							<!-- Text Content -->
							<div>
								<h3 class="font-black text-base sm:text-lg leading-tight mb-2 group-hover:text-red-600 sm:text-white transition-colors line-clamp-2 sm:line-clamp-none text-slate-900 dark:text-white">
									{event.title}
								</h3>

								<div class="space-y-1.5">
									{#if (event.seitansaiMembers?.length ?? 0) > 0}
										<div class="flex items-center gap-2 text-[10px] sm:text-xs text-pink-600 sm:text-pink-300 font-black uppercase tracking-wider">
									...
									<div class="flex flex-col gap-1 sm:text-white/80">
										<div class="flex items-center gap-2 text-[10px] sm:text-xs font-bold uppercase tracking-widest">
											<Calendar class="w-3.5 h-3.5" />
											<span>{$formatDate(event.date, { day: 'numeric', month: 'short', year: 'numeric' })}</span>
										</div>
										{#if event.setlistId}
											<div class="flex items-center gap-2 text-[10px] sm:text-xs font-bold uppercase tracking-widest">
