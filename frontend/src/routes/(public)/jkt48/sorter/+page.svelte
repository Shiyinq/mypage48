...
>
	{#if currentState === 'landing'}
		...
		<div in:fade={{ duration: 300 }} class="w-full max-w-2xl space-y-6">
			<div
				class="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-xl border border-gray-100 dark:border-zinc-800 space-y-6"
			>
				<div class="flex items-center justify-between">
					<h3 class="font-black text-themed uppercase tracking-widest text-sm text-slate-400">
						{$t('theater.sorter.generation')}
					</h3>
					<div class="flex gap-4 font-black">
						<button
							on:click={selectAllGenerations}
							class="text-xs text-red-600 hover:scale-105 transition-transform cursor-pointer uppercase tracking-widest"
							>{$t('theater.sorter.selectAll')}</button
						>
						<button
							on:click={deselectAllGenerations}
							class="text-xs text-slate-400 hover:text-themed transition-colors cursor-pointer uppercase tracking-widest"
							>{$t('theater.sorter.clear')}</button
						>
					</div>
				</div>
				...
				<div
					class="pt-6 border-t border-gray-50 dark:border-zinc-800 flex items-center justify-between"
				>
					...
					<div class="flex items-baseline gap-2">
						...
						<span class="text-[10px] font-black uppercase border-b-2 border-red-600"
			...
			<button
				...
				disabled={loadingGenerations ||
					allMembers.filter((m) => selectedGenerations.has(m.generation)).length < 2}
		...
		<div in:fade class="w-full max-w-2xl space-y-6 flex flex-col items-center">
			<div class="w-full space-y-2">
				<div class="flex justify-between items-end px-2">
					<div class="space-y-0.5">
						<h2
							class="text-slate-900 dark:text-white font-black text-lg uppercase tracking-tighter"
						>
							{$t('theater.sorter.sorting')}
						</h2>
						<p class="text-slate-400 text-[8px] font-black uppercase tracking-widest">
							{$t('theater.sorter.questionLabel', { num: numQuestion })}
						</p>
					</div>
					<div class="text-right">
						<span class="text-red-600 font-black text-2xl italic tracking-tighter"
				...
				>
					<div
						class="h-full bg-gradient-to-r from-red-500 to-red-600 transition-all duration-500 ease-out rounded-full shadow-lg shadow-red-500/40"
			...
			<div class="grid grid-cols-[1fr_auto_1fr] items-center gap-4 md:gap-8 w-full max-w-2xl">
				<button
					...
					class={`group relative aspect-[2/3] rounded-xl overflow-hidden border-4 border-transparent hover:border-red-600 transition-all hover:shadow-2xl hover:shadow-red-500/20 active:scale-95 bg-slate-100 dark:bg-zinc-800 cursor-pointer shadow-xl ${
				...
				>
					...
					/>
					<img
						src={leftMember?.member_type?.toLowerCase() === 'trainee'
							? TRAINEE_FRAME
							: MEMBER_FRAME}
						alt="frame"
					...
					<div class="absolute bottom-3 left-3 right-3 text-left z-30">
						<span
							class="px-1.5 py-0.5 bg-red-600 text-white text-[7px] font-black rounded-md uppercase tracking-widest mb-1 block w-fit"
							>Gen {leftMember?.generation}</span
						>
						<h3 class="text-white text-xs font-black leading-tight drop-shadow-md truncate">
							{leftMember?.name}
						</h3>
					</div>
					...
					{#if lastSelectedSide === 'left' || lastSelectedSide === 'tie'}
						<div class="absolute inset-0 flex items-center justify-center z-50 pointer-events-none">
							<Heart class="text-red-500 fill-current w-16 h-16 heart-float" />
						</div>
				...
				<div
					class={`z-10 w-10 h-10 flex items-center justify-center bg-white dark:bg-zinc-900 rounded-full shadow-2xl border-2 border-red-600 text-red-600 font-black italic text-[10px] ${isAnimating ? 'vs-pulse' : 'animate-pulse'}`}
				>
					VS
				</div>
				...
				>
					...
					/>
					<img
						src={rightMember?.member_type?.toLowerCase() === 'trainee'
							? TRAINEE_FRAME
							: MEMBER_FRAME}
						alt="frame"
						class="absolute inset-0 w-full h-full object-fill pointer-events-none z-10"
					...
					<div class="absolute bottom-3 left-3 right-3 text-left z-30">
						<span
							class="px-1.5 py-0.5 bg-red-600 text-white text-[7px] font-black rounded-md uppercase tracking-widest mb-1 block w-fit"
							>Gen {rightMember?.generation}</span
						>
						<h3 class="text-white text-xs font-black leading-tight drop-shadow-md truncate">
							{rightMember?.name}
						</h3>
		...
		>
			<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
				<div class="flex items-center gap-4">
					<div
						class="w-12 h-12 rounded-2xl bg-red-600 flex items-center justify-center text-white shadow-xl shadow-red-500/20 shrink-0"
					>
						<Trophy size={22} />
					</div>
					<div class="space-y-0.5">
						<h1
							class="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none"
						>
				...
				<div class="flex items-center gap-2">
					<div
						class="flex bg-white dark:bg-zinc-900 rounded-full p-1 border border-gray-100 dark:border-zinc-800 shadow-sm mr-2"
					>
						<button
					...
					<button
						...
						class="h-11 px-6 bg-red-600 hover:bg-red-700 text-white font-black rounded-full transition-all shadow-lg shadow-red-500/20 flex items-center gap-2 text-xs cursor-pointer"
					>
						<Share2 size={16} />
					...
					<button
						on:click={restart}
						class="h-11 px-6 bg-white dark:bg-zinc-800 text-slate-900 dark:text-white font-black rounded-full transition-all shadow-md border border-gray-100 dark:border-zinc-700 flex items-center gap-2 text-xs cursor-pointer"
					>
						<RotateCcw size={16} />
							...
							{#each results.slice(0, 3) as member, i (member.id)}
								...
								>
									...
									<div class="flex flex-col gap-0.5 z-30">
										<h4
											class="font-black text-slate-900 dark:text-white text-base md:text-lg uppercase tracking-tight leading-none truncate max-w-[200px] sm:max-w-none"
										>
											{member.name}
										</h4>
										<span
											class="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest truncate"
											>{member.generation.length > 5
												? member.generation
												: 'Gen ' + member.generation}</span
										>
									</div>
							...
							{#if results.length > 3}
								<div class="grid grid-cols-1 md:grid-cols-2 gap-3 w-full mt-1">
									{#each results.slice(3) as member, i (member.id)}
										...
										>
											<div
												class="rank-badge w-8 h-8 rounded-full flex items-center justify-center font-black text-xs shrink-0 z-30 bg-slate-100 dark:bg-zinc-800 text-slate-900 dark:text-white border border-slate-200 dark:border-zinc-700 shadow-sm"
											>
												{i + 4}
											</div>
											...
											<div class="flex flex-col gap-0.5 z-30 min-w-0">
												<h4
													class="font-black text-slate-900 dark:text-white text-xs sm:text-sm uppercase tracking-tight leading-none truncate"
												>
													{member.name}
												</h4>
												<span
...
<style>
	...
	@keyframes heart-break {
		...
		100% {
			transform: scale(0.7) translateY(80px) rotate(25deg);
			opacity: 0;
		}
	}
</style>
