...
<div class="max-w-4xl mx-auto space-y-8 animate-fade-in pb-20">
    <!-- Header / Back -->
    <div class="space-y-6">
        <a href="/jkt48/news" class="inline-flex items-center gap-2 text-sm font-bold text-slate-400 hover:text-red-500 transition-colors group">
            <MoveLeft class="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
            {$t('common.back') || 'Back'}
        </a>
        
        <h1 class="text-3xl md:text-5xl font-black text-slate-900 dark:text-white leading-tight uppercase tracking-tight">
            {item.title}
        </h1>
    ...
    <main class="space-y-8">
        ...
        <!-- Share Actions -->
        <div class="flex flex-col items-center gap-6 py-12 border-t border-slate-100 dark:border-white/10 mt-12">
        ...
        <div class="flex justify-center pb-20">
            ...
            >
                {$t('theater.news.readOriginal')} <ExternalLink class="w-5 h-5" />
            </a>
        </div>
    </main>
    ...
    {#if recentNews.length > 0}
        <section class="space-y-8 pt-12 border-t border-slate-100 dark:border-white/10">
            <div class="flex items-center justify-between">
                <h3 class="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{$t('theater.news.otherNews')}</h3>
                <a href="/jkt48/news" class="text-sm font-bold text-red-500 hover:text-red-600 flex items-center gap-1">
                    {$t('theater.news.seeAll')} <ChevronRight class="w-4 h-4" />
                </a>
            </div>
                ...
                {#each recentNews as recent}
                    ...
                    >
                        <div class="flex items-center gap-3 mb-4">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm {recent.category.toLowerCase() === 'event' ? 'bg-red-600 text-white' : recent.category.toLowerCase() === 'theater' ? 'bg-cyan-600 text-white' : 'bg-orange-600 text-white'}">
                                {recent.category}
                            </span>
                        ...
                        <h4 class="text-base font-black text-slate-900 dark:text-slate-100 leading-tight group-hover:text-red-500 transition-colors line-clamp-2 uppercase">
                            {recent.title}
                        </h4>
                    </a>
                {/each}
