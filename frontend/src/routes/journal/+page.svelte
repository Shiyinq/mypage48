<script lang="ts">
	...
	$: hasMore = state.pagination ? state.pagination.current_page < state.pagination.last_page : false;
...
<div class="h-[calc(100vh-64px)] flex flex-col bg-slate-50/50 dark:bg-zinc-900/40 overflow-hidden relative overscroll-none">
	...
	{:else if error && tickets.length === 0}
		<div class="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-6 pb-32" in:fade>
	...
	{:else}
		<div class="flex-1 flex flex-col md:flex-row overflow-hidden relative" in:fade>
			...
			<div class="flex-1 flex flex-col md:flex-row overflow-hidden relative">
				...
				{#if !isSidebarVisible}
					<div class="absolute top-4 left-0 z-[50] transition-all duration-300" transition:fade={{ duration: 200 }}>
						<button 
				...
				<div class="flex-1 min-h-0 min-w-0 overflow-hidden flex relative">
					{#if !selectedTicket}
						<div class="flex-1 flex flex-col items-center justify-center p-12 text-center relative pb-32">
							<div class="w-24 h-24 bg-gray-50 dark:bg-zinc-900/50 rounded-full flex items-center justify-center mb-6">
								<PenLine class="w-10 h-10 text-gray-300 dark:text-gray-700" />
							</div>
							<h2 class="text-2xl font-black text-gray-900 dark:text-gray-100 mb-2">{$t('journal.emptyTitle')}</h2>
