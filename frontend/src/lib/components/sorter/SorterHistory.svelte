<script lang="ts">
	import { Calendar, Trash2, Eye, History } from 'lucide-svelte';
	...
	interface Props {
		histories: SorterResponse[];
		loading: boolean;
		onview?: (item: SorterResponse) => void;
		ondelete?: (id: string) => void;
	}

	let { histories, loading, onview, ondelete }: Props = $props();
...
<div in:fade={{ duration: 300 }} class="w-full max-w-5xl mx-auto px-1 pb-12">
	...
	{:else if histories.length === 0}
		...
		>
			...
			<p class="text-xs font-medium text-zinc-400 dark:text-zinc-500 max-w-sm leading-relaxed mb-6">
	...
	{:else}
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
			{#each histories as item, i (item._id)}
