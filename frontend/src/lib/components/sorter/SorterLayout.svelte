<script lang="ts">
	...
	const isBackIcon = $derived(
		sorterNavbarStore.pageType === 'history-detail' ||
			(sorterNavbarStore.pageType === 'sorter' && sorterNavbarStore.sorterState === 'sorting')
	);
...
<div class="flex flex-col min-h-screen w-full relative bg-pink-50/30 dark:bg-zinc-950">
	...
	<!-- Main Sorter Navbar -->
	<div
		class="fixed top-0 left-0 right-0 w-full z-[50] border-b border-slate-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl"
	>
		<div
	...
	<nav
		class="sm:hidden fixed bottom-0 left-0 right-0 z-[50] backdrop-blur-xl border-t pb-safe shadow-[0_-4px_20px_rgba(0,0,0,0.03)] dark:shadow-none transition-all duration-300 ease-in-out bg-white/80 dark:bg-zinc-950/80 border-slate-200 dark:border-zinc-800"
	>
		<div
