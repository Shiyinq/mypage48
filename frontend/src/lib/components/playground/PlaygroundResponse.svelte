...
<div class="flex flex-col h-full bg-gray-50 dark:bg-zinc-950/20 border-l border-gray-100 dark:border-white/5 w-[35%] min-w-[450px] shrink-0 p-6 overflow-hidden">
	<div class="flex items-center justify-between mb-6">
		...
		{#if duration}
			<div class="flex items-center gap-1.5 px-3 py-1 bg-white dark:bg-zinc-800 rounded-full border border-gray-100 dark:border-white/5 text-[10px] font-bold text-gray-500 dark:text-gray-400">
				<Clock class="w-3 h-3" />
				{duration}ms
			</div>
		{/if}
	...
	{#if response || error}
		<div in:fade={{ duration: 200 }} class="flex-1 flex flex-col space-y-4 overflow-hidden">
			<!-- Status -->
			<div class="flex items-center gap-3 p-4 rounded-2xl border {isSuccess ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/20 text-emerald-700 dark:text-emerald-400' : 'bg-rose-50 dark:bg-rose-900/10 border-rose-100 dark:border-rose-900/20 text-rose-700 dark:text-rose-400'}">
				{#if isSuccess}
				...
				<div class="flex flex-col">
					<span class="text-sm font-black uppercase tracking-widest">{response?.status || error?.status || 'Error'}</span>
					<span class="text-xs font-bold opacity-70">{statusText}</span>
				</div>
			</div>

			<!-- Body -->
			<div class="flex-1 flex flex-col bg-gray-900 dark:bg-zinc-950 rounded-3xl border border-white/5 overflow-hidden relative">
				<div class="flex items-center justify-between px-4 py-3 bg-white/5 border-b border-white/5 backdrop-blur-md">
				...
				<div class="flex-1 overflow-y-auto p-4 custom-scrollbar">
					<pre class="font-mono text-[11px] leading-relaxed {isSuccess ? 'text-emerald-400' : 'text-rose-400'}">{formattedJson || errorJson || '// No content'}</pre>
				</div>
			</div>
		</div>
	{:else}
