...
{#if activeGift}
	{#key activeGift.timestamp}
		...
		>
			...
			<div class="absolute top-4 left-1/2 -translate-x-1/2 flex items-center justify-center w-full px-4">
				<div 
					...
					style="background: {activeGift.gift.color ? `linear-gradient(to right, ${activeGift.gift.color}EE, ${activeGift.gift.color}AA)` : 'linear-gradient(to right, #f97316EE, #ef4444AA)'}"
				>
					...
					{/if}
					<div class="text-left">
						<p class="text-[9px] font-black text-white leading-none whitespace-nowrap drop-shadow-sm">{activeGift.user}</p>
						<p class="text-[7px] font-bold text-white/90 leading-tight">mengirim {activeGift.gift.name}</p>
					</div>
				</div>
			</div>
			...
			>
				...
				{#if activeGift.gift.img}
					<div class="relative flex items-center justify-center p-8 scale-90 sm:scale-110 md:scale-125">
						...
						{/if}
						<!-- Glow Effect -->
						<div class="absolute inset-0 bg-white/5 blur-[40px] rounded-full animate-pulse z-[-1]"></div>
					</div>
				{/if}
			</div>
		</div>
