...
<div
	class="shadow-sm hover:shadow-xl transition-all duration-300 w-full group transform-gpu rounded-2xl"
>
	<div
		class="ticket-mask bg-white dark:bg-zinc-900 rounded-2xl overflow-hidden border border-gray-100 dark:border-zinc-700 flex flex-row h-[210px] w-full"
	>
		<!-- Image Section (Left) -->
		...
		<!-- Dotted Divider Line -->
		<div
			class="w-0 border-l-[2px] border-dotted border-gray-300 dark:border-zinc-600 my-3 relative z-10"
		></div>
			...
			<div class="p-3 pb-0">
				<div class="flex justify-between items-start gap-2">
					<div class="flex-1 min-w-0">
						<div
							class="text-gray-400 dark:text-gray-500 font-mono text-[10px] mb-0.5 tracking-wide"
						>
							#{ticket.ticket_id}
						</div>
						<h3
...
<style>
	.ticket-mask {
		...
		-webkit-mask-image:
			radial-gradient(
				circle at var(--mask-pos) 0,
				transparent var(--cutout-size),
				black calc(var(--cutout-size) + 0.5px)
			),
			radial-gradient(
				circle at var(--mask-pos) 100%,
				transparent var(--cutout-size),
				black calc(var(--cutout-size) + 0.5px)
			);
		-webkit-mask-size:
