...
<div class="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6">
	...
	<!-- Modal Content -->
	<div
		class="bg-white/95 backdrop-blur-xl w-full max-w-5xl h-[90vh] rounded-3xl shadow-2xl overflow-y-auto custom-scrollbar relative z-10 animate-fade-in"
	>
			...
			<div class="flex items-center justify-between mb-6">
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<p class="text-sm text-gray-500 mt-1">Update your memory</p>
					</div>
				</div>
			...
			<div class="grid gap-8 lg:grid-cols-2">
				...
				<div class="flex flex-col gap-4">
					<div class="sticky top-6">
						{#if image}
							<div
								class="relative rounded-3xl overflow-hidden border border-gray-200 bg-gray-100 shadow-lg aspect-[4/5] lg:aspect-auto lg:h-[calc(100vh-300px)] group"
							>
								...
								>
									...
									>
										<ImagePlus class="w-4 h-4" /> Change Photo
									</button>
								</div>
							</div>
						{:else}
							<div
								on:click={() => fileInputRef.click()}
								class="rounded-3xl border-3 border-dashed border-gray-200 bg-gray-50 hover:bg-red-50 hover:border-red-200 transition-all cursor-pointer flex flex-col items-center justify-center aspect-[4/5] lg:aspect-auto lg:h-[calc(100vh-300px)] text-gray-400 hover:text-red-500"
							>
					...
					<form on:submit|preventDefault={handleSubmit} class="space-y-8">
						...
						<div class="pt-4">
							<button
								...
								class="w-full idol-gradient text-white py-4 rounded-2xl font-bold text-lg shadow-lg shadow-red-200 hover:shadow-xl hover:scale-[1.01] transition-all flex items-center justify-center gap-2 disabled:opacity-50"
							>
								{#if isSubmitting}
