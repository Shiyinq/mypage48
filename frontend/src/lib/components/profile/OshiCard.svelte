...
>
	{#if loading}
		<!-- Banner for Loading -->
		<div
			class="h-32 w-full rounded-t-3xl overflow-hidden relative bg-gray-200 dark:bg-zinc-800 animate-pulse"
		></div>
	...
	{:else if currentOshi}
		...
		<div class="relative w-full rounded-t-3xl">
			<!-- Banner Background Behind Content -->
			<div
				class="absolute inset-0 z-0 rounded-t-3xl overflow-hidden pointer-events-none"
			...
			>
				{#key currentOshi?.memberType}
					{@const isDefault = !['LOVE', 'DREAM', 'PASSION'].includes(
						currentOshi?.memberType?.toUpperCase() || ''
					)}
					<div
						transition:fade={{ duration: 400 }}
						class="absolute inset-0 {isDefault
							? 'bg-[length:50%_auto] bg-repeat bg-center md:bg-cover md:bg-[center_30%] md:bg-no-repeat'
			...
			<!-- Top Profile Content (Avatar, Info, Socials) -->
			<div class="relative z-10 px-6 md:px-8 pt-14 pb-2">
		...
		<div class="px-6 md:px-8 pb-6 relative z-10">
			<!-- Stats Grid -->
			<div
				class="grid grid-cols-3 gap-2 md:gap-3 border-t border-gray-100 dark:border-zinc-800 pt-4"
			>
				<div
	...
	{#if oshis.length > 1}
		<button
			onclick={goPrev}
			class="absolute left-2 top-1/2 -translate-y-1/2 z-10 bg-black/30 hover:bg-black/50 text-white rounded-full p-1.5 transition-all duration-200 cursor-pointer {!isHovering
				? 'opacity-0'
				: 'opacity-100'}"
		...
		<button
			onclick={goNext}
			class="absolute right-2 top-1/2 -translate-y-1/2 z-10 bg-black/30 hover:bg-black/50 text-white rounded-full p-1.5 transition-all duration-200 cursor-pointer {!isHovering
				? 'opacity-0'
				: 'opacity-100'}"
