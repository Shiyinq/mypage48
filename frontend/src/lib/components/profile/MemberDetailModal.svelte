...
{#if show}
	<div class="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-hidden" use:portal>
		...
		>
			...
			{#if members.length > 0}
				...
				>
				<div class="p-4 pb-2 flex items-center justify-around border-b border-gray-100 dark:border-zinc-800/50">
					<button 
						class="px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-300 cursor-pointer {activeTab === 'Member' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-400 dark:text-zinc-500 hover:text-gray-600'}"
						on:click={() => switchTab('Member')}
					>
						Member
					</button>
					<button 
						class="px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-300 cursor-pointer {activeTab === 'Trainee' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-400 dark:text-zinc-500 hover:text-gray-600'}"
				...
				>
					<div class="flex flex-row md:flex-col gap-1.5 md:space-y-1">
						{#each displayMembers as m (m.id)}
								<button
									class="text-left px-3 md:px-4 py-1.5 md:py-2 rounded-xl md:rounded-2xl transition-all duration-300 group relative flex items-center justify-between cursor-pointer select-none shrink-0 md:shrink {internalMemberId === m.id
								...
								>
									<div class="flex items-center gap-2 md:gap-3 whitespace-nowrap">
										{#if internalMemberId === m.id}
											<div class="w-1 md:w-1.5 h-1 md:h-1.5 rounded-full bg-white animate-pulse"></div>
										{/if}
			...
			<!-- Main Content -->
			<div class="flex-1 flex flex-col md:flex-row overflow-y-auto custom-scrollbar md:overflow-hidden relative bg-white dark:bg-zinc-900">
				{#if loading}
				...
				{:else if currentMember}
					...
					<div class="flex-none md:flex-1 md:w-1/2 relative bg-white dark:bg-zinc-900 md:border-l border-gray-100 dark:border-zinc-800 shadow-[-10px_0_30px_rgba(0,0,0,0.02)] flex flex-col">
						...
						<div class="flex-1 flex flex-col">
							{#key currentMember.id}
								...
								>
									...
									<div class="space-y-1">
										<div class="flex items-center gap-3">
											<span class="px-2 py-0.5 rounded-md bg-red-600 text-[10px] font-black text-white uppercase tracking-wider">
								...
								<!-- Jikoshoukai -->
								<div class="bg-gray-50 dark:bg-zinc-800/30 p-6 rounded-[28px] relative border border-gray-100 dark:border-zinc-800/50 group transition-all duration-500">
									<Quote class="w-8 h-8 text-red-500/10 dark:text-red-900/20 absolute -top-3 -left-2 transform -scale-x-100" />
									<p class="text-sm md:text-base text-gray-700 dark:text-gray-300 italic text-center leading-relaxed font-medium">
										"{currentMember.jiko}"
									</p>
								</div>
								...
								<div class="grid grid-cols-2 gap-4">
									<div class="bg-gray-50/50 dark:bg-zinc-800/20 p-4 rounded-2xl border border-gray-100 dark:border-zinc-800/50 group hover:border-red-200 dark:hover:border-red-900/30 transition-all duration-300">
										<p class="text-[9px] font-black text-gray-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 group-hover:text-red-500 transition-colors">{$t('member.birthdate')}</p>
										<p class="text-sm font-black text-themed leading-tight">
											{$formatDate(parseIndonesianDate(currentMember.birthdate), { dateStyle: 'medium' })}
											<span class="text-[10px] text-gray-500 dark:text-zinc-400 font-bold block mt-1 px-2 py-0.5 bg-gray-200/50 dark:bg-zinc-700/50 w-max rounded-full">{calculateAge(currentMember.birthdate)} {$t('member.yearsOld')}</span>
										</p>
									</div>
									<div class="bg-gray-50/50 dark:bg-zinc-800/20 p-4 rounded-2xl border border-gray-100 dark:border-zinc-800/50 group hover:border-red-200 dark:hover:border-red-900/30 transition-all duration-300">
										<p class="text-[9px] font-black text-gray-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 group-hover:text-red-500 transition-colors">{$t('member.horoscope')}</p>
										<p class="text-sm font-black text-themed leading-tight">{currentMember.horoscope}</p>
									</div>
									<div class="bg-gray-50/50 dark:bg-zinc-800/20 p-4 rounded-2xl border border-gray-100 dark:border-zinc-800/50 group hover:border-red-200 dark:hover:border-red-900/30 transition-all duration-300">
										<p class="text-[9px] font-black text-gray-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 group-hover:text-red-500 transition-colors">{$t('member.bloodType')}</p>
										<p class="text-sm font-black text-themed leading-tight">{currentMember.bloodType}</p>
									</div>
									<div class="bg-gray-50/50 dark:bg-zinc-800/20 p-4 rounded-2xl border border-gray-100 dark:border-zinc-800/50 group hover:border-red-200 dark:hover:border-red-900/30 transition-all duration-300">
										<p class="text-[9px] font-black text-gray-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 group-hover:text-red-500 transition-colors">{$t('member.height')}</p>
										<p class="text-sm font-black text-themed leading-tight">
											{currentMember.height?.toString().toLowerCase().includes('cm') ? currentMember.height : currentMember.height + 'cm'}
										</p>
									</div>
								</div>
								</div>
								...
								>
									{#if currentMember.socials.twitter}
										<a href={currentMember.socials.twitter} target="_blank" rel="noopener noreferrer" class="p-2.5 bg-gray-50 dark:bg-zinc-800 rounded-2xl text-gray-500 dark:text-zinc-400 hover:bg-black hover:text-white transition-all duration-500 hover:-translate-y-1 shadow-sm"><svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>
									{/if}
									{#if currentMember.socials.instagram}
										<a href={currentMember.socials.instagram} target="_blank" rel="noopener noreferrer" class="p-2.5 bg-gray-50 dark:bg-zinc-800 rounded-2xl text-gray-500 dark:text-zinc-400 hover:bg-pink-600 hover:text-white transition-all duration-500 hover:-translate-y-1 shadow-sm"><Instagram class="w-4 h-4" /></a>
									{/if}
									{#if currentMember.socials.tiktok}
										<a href={currentMember.socials.tiktok} target="_blank" rel="noopener noreferrer" class="p-2.5 bg-gray-50 dark:bg-zinc-800 rounded-2xl text-gray-500 dark:text-zinc-400 hover:bg-black hover:text-white transition-all duration-500 hover:-translate-y-1 shadow-sm font-bold">
											<svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.06-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.1-3.34-3.12-3.35-5.47-.03-2.43 1.4-4.71 3.61-5.7 1.11-.53 2.33-.78 3.56-.7v4.26c-.15-.05-.31-.07-.46-.09-1.49-.22-3.08.75-3.39 2.22-.2 1.05.21 2.18 1.03 2.87.89.73 2.15.8 3.2.4 1.18-.5 1.88-1.76 1.87-3.01.01-6.19-.01-12.38.01-18.57z"/></svg>
										</a>
									{/if}
									{#if currentMember.socials.idn_app}
										<a href={currentMember.socials.idn_app} target="_blank" rel="noopener noreferrer" class="p-2.5 bg-gray-50 dark:bg-zinc-800 rounded-2xl text-gray-500 dark:text-zinc-400 hover:bg-red-600 hover:text-white transition-all duration-500 hover:-translate-y-1 shadow-sm"><Smartphone class="w-4 h-4" /></a>
									{/if}
									{#if currentMember.socials.showroom}
										<a href={currentMember.socials.showroom} target="_blank" rel="noopener noreferrer" class="p-2.5 bg-gray-50 dark:bg-zinc-800 rounded-2xl text-gray-500 dark:text-zinc-400 hover:bg-blue-600 hover:text-white transition-all duration-500 hover:-translate-y-1 shadow-sm"><Tv class="w-4 h-4" /></a>
									{/if}
									{#if currentMember.href}
										<a href={currentMember.href.startsWith('http') ? currentMember.href : `https://jkt48.com${currentMember.href}`} target="_blank" rel="noopener noreferrer" class="p-2.5 bg-gray-50 dark:bg-zinc-800 rounded-2xl text-gray-500 dark:text-zinc-400 hover:bg-red-700 hover:text-white transition-all duration-500 hover:-translate-y-1 shadow-sm" title="Official Profile"><Globe class="w-4 h-4" /></a>
									{/if}
								</div>
							{/key}
						</div>
				...
				{:else}
					<div class="col-span-2 p-12 flex flex-col items-center justify-center min-h-[400px] text-center">
						<div class="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mb-4"><Search class="w-8 h-8 text-red-500" /></div>
						...
						<button class="px-6 py-2.5 bg-gray-900 text-white rounded-xl hover:bg-black transition-colors font-medium cursor-pointer" on:click={onClose}>{$t('member.close')}</button>
					</div>
				{/if}
			</div>
		</div>
