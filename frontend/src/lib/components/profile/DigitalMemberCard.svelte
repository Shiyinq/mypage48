...
<div class="relative group perspective-1000 w-full min-w-0">
	<!-- Card Container -->
	<div
		class="relative h-56 w-full rounded-3xl shadow-2xl transition-transform duration-500 group-hover:scale-[1.02] {profile?.ofcStatus === 'Active' ? 'ring-2 ring-green-400/50 ring-offset-2 ring-offset-white' : ''} max-w-full"
	>
		...
		<!-- Card Content -->
		<div class="relative z-10 p-4 sm:p-6 h-full flex flex-col justify-between text-white overflow-hidden">
			<!-- Top Row -->
			<div class="flex justify-between items-start gap-2 min-w-0">
				...
				<div class="flex flex-col items-end gap-1 min-w-0">
					...
					<!-- OFC Status Badge -->
					{#if loading}
						<div class="h-4 sm:h-5 w-12 sm:w-16 bg-white/20 rounded-full animate-pulse mt-1"></div>
					{:else}
						<div
						...
						>
							<span
								class="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full flex-shrink-0 {profile?.ofcStatus === 'Active'
			...
			<div class="flex items-center gap-3 sm:gap-4 my-1 sm:my-2 opacity-80 min-w-0">
				...
				<!-- WifiIcon -->
				<svg viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 sm:w-6 sm:h-6 rotate-90 text-gray-500 flex-shrink-0">
					<path
			...
			<div class="flex justify-between items-end gap-2 min-w-0">
				<div class="min-w-0">
					...
					<p class="text-base sm:text-lg font-bold tracking-wide uppercase text-shadow-sm truncate">
						{#if loading}
							<span class="inline-block w-32 sm:w-40 h-4 sm:h-5 bg-white/20 rounded animate-pulse"></span>
						{:else}
							{profile?.name || 'N/A'}
						{/if}
					</p>
