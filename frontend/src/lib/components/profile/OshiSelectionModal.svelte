<script lang="ts">
	import { Search, X, Check, User } from 'lucide-svelte';
						...
						{#each memberList as member (member.id)}
							...
							>
								<div class="relative w-14 h-14 md:w-20 md:h-20 mb-2 md:mb-3">
									...
									{:else}
										<div
											class="w-full h-full rounded-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-zinc-800 dark:to-zinc-700 flex items-center justify-center shadow-sm group-hover:shadow-md transition-shadow {isSelected(
												member.id
											)
												? 'ring-2 ring-red-500 ring-offset-2 dark:ring-offset-zinc-900'
												: ''}"
