...
<div class="h-full flex flex-col bg-slate-50/30 dark:bg-zinc-900/50">
	<div class="p-3 border-b border-gray-100 dark:border-white/5 sticky top-0 bg-white/95 dark:bg-zinc-950/95 backdrop-blur z-10 flex-shrink-0 flex justify-between items-center">
        ...
        <div class="flex items-center gap-1">
            <div class={`text-[10px] bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-gray-400 font-bold px-2 py-0.5 rounded-full flex items-center gap-1.5 transition-all ${loading ? 'opacity-70 ring-1 ring-red-500/10' : ''}`}>
                {#if loading}
                    <div class="w-2 h-2 border-2 border-red-500/30 border-t-red-500 rounded-full animate-spin"></div>
                {/if}
                {totalData || tickets.length} {$t('shows.unit')}
            </div>
            <button 
    ...
    <div class="px-3 py-2 border-b border-gray-100 dark:border-white/5 bg-white/50 dark:bg-zinc-950/50 relative z-20">
        <HistoryFilter 
            {filters} 
	...
	<div class="flex-1 overflow-y-auto px-2 py-3 custom-scrollbar relative">
		{#if loading && tickets.length === 0}
			<div class="space-y-2">
				{#each Array(5) as _}
					<div class="animate-pulse bg-white/50 dark:bg-zinc-800/50 h-24 rounded-xl border border-gray-100 dark:border-white/5 w-full"></div>
				{/each}
			</div>
		...
		{:else}
			<div class="space-y-1 flex flex-col pb-10">
				{#each tickets as ticket}
					...
					>
						<div class="flex items-center justify-between mb-1">
                            <div class="text-[9px] font-bold tracking-wider uppercase {isSelected ? 'text-red-500' : 'text-gray-400 dark:text-gray-500'}">
                                {$formatDate(ticket.event.date, { day: 'numeric', month: 'short', year: 'numeric' })}
                            </div>
                            <div class="flex items-center gap-0.5 font-mono text-[9px] font-bold {isSelected ? 'text-red-500/80' : 'text-gray-400'}">
						...
						<div class="flex items-start justify-between gap-2">
                            <h3 class={`text-xs font-bold leading-tight line-clamp-2 transition-colors flex-1 ${isSelected ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white'}`}>
                                {ticket.event.title}
                            </h3>
                            {#if ticket.notes}
                                 <span class="mt-1 w-1.5 h-1.5 rounded-full bg-green-500 shrink-0 shadow-[0_0_8px_rgba(34,197,94,0.4)]" title="Journal Written"></span>
                            {/if}
                        </div>
					</button>
				{/each}
                ...
                {#if hasMore}
                    <div use:infiniteScroll on:intersect={() => dispatch('loadMore')} class="w-full py-4 flex justify-center">
                        {#if loading}
                            <span class="w-4 h-4 border-2 border-red-500 border-t-transparent rounded-full animate-spin"></span>
                        {/if}
                    </div>
                {/if}
			</div>
