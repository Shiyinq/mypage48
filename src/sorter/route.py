...
from src.sorter.schemas import SorterCreateRequest, SorterResponse
...
@router.get("", response_model=List[SorterResponse])
async def get_sorters(
    current_user: UserCurrent = Depends(get_current_user),
    service: SortersService = Depends(get_sorters_service),
):
    return await service.get_sorters(user_id=current_user.userId)
