    ...
    async def get_status(self, user_id: str) -> ExportResponse:
        ...
        if job.status == ExportStatus.PROCESSING:
             if datetime.utcnow() - job.updated_at > timedelta(hours=1):
                 await self.export_repo.update_status(user_id, ExportStatus.FAILED, error="Timeout")
                 return ExportResponse(status=ExportStatus.FAILED, message="Job timed out")
    ...
    async def initiate_export(self, user_id: str) -> ExportResponse:
        ...
        if job:
            ...
            if job.status == ExportStatus.COMPLETED:
                 ...
                 expires_at = job.updated_at + timedelta(hours=24)
                 if datetime.utcnow() < expires_at:
                      return ExportResponse(status=ExportStatus.COMPLETED, expires_at=expires_at)
                 else:
                      # Expired, allow new
                      pass
    ...
    async def _process_export(self, user_id: str):
        try:
            ...
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                ...
                def add_file_to_zip(path: str, zip_path_prefix: str, custom_name: str = None):
                    if not path:
                        return
                    
                    # Avoid logging long strings (likely base64)
                    if len(path) > 256:
                        logger.info(f"Processing image (len={len(path)}) for {zip_path_prefix}")
                    else:
                        logger.info(f"Attempting to add file: {path} -> {zip_path_prefix}")
                    ...
                    try:
                        ...
                        if path.startswith("data:") or len(path) > 500:
                            try:
                                ...
                                if "base64," in path:
                                    ...
                                    if "image/png" in header: fname = custom_name or "image.png"
                                    elif "image/jpeg" in header: fname = custom_name or "image.jpg"
                                ...
                                if not custom_name:
                                    ...
                                    name_hash = hashlib.md5(encoded.encode()).hexdigest()[:10]
                        ...
                        # Case 2: External URL (HTTP/HTTPS)
                             ...
                             if res.status_code == 200:
                                 ...
                                 logger.warning(f"Failed to download {path}: {res.status_code}")
                                 return
                        ...
                        if file_data:
                            ...
                            if not fname or fname == ".": fname = "image.png"
                ...
                for t in tickets:
                    ...
                    if t.get("two_shot") and t["two_shot"].get("imageUrl"):
                         ...
                         if len(val) < 500 and "/" in val:
                             ...
                             member = t["two_shot"].get("member_name", "member").replace(" ", "_")
            ...
            # 4. Upload Zip
            zip_filename = f"exports/{user_id}/{datetime.utcnow().strftime('%Y%m%d%H%M%S')}.zip"
            self.storage_repo.upload_file(zip_buffer.getvalue(), zip_filename, content_type="application/zip")
            
            # 5. Update Status
            await self.export_repo.update_status(user_id, ExportStatus.COMPLETED, file_path=zip_filename)
            logger.info(f"Export completed for user {user_id}")
        
        except Exception as e:
            logger.exception(f"Export failed for user {user_id}: {e}")
            await self.export_repo.update_status(user_id, ExportStatus.FAILED, error=str(e))

    async def download_export(self, user_id: str): # Returns tuple: (stream, filename, cleanup_func)
        job = await self.export_repo.get_job(user_id)
