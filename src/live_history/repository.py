...
class LiveHistoryRepository:
    ...
    async def get_history_by_user(self, user_id: str, skip: int = 0, limit: int = 20) -> List[Dict[str, Any]]:
        cursor = self.collection.find({"user_id": user_id}).sort("last_updated_at", -1).skip(skip).limit(limit)
        return [doc async for doc in cursor]
        
    async def get_history_by_user_and_member(self, user_id: str, member_id: str, skip: int = 0, limit: int = 20) -> List[Dict[str, Any]]:
        cursor = self.collection.find({"user_id": user_id, "member_id": member_id}).sort("last_updated_at", -1).skip(skip).limit(limit)
        return [doc async for doc in cursor]

    async def get_total_history_count(self, user_id: str, member_id: Optional[str] = None) -> int:
    ...
    async def get_longest_watch(self, user_id: str, member_id: str = None) -> Optional[Dict[str, Any]]:
        query = {"user_id": user_id}
        if member_id:
            query["member_id"] = member_id
        
        longest_doc = await self.collection.find_one(
            query,
            sort=[("duration", -1)]
        )
