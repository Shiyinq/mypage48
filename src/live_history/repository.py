...
class LiveHistoryRepository:
    ...
    async def get_pc_collection(
        ...
        watched_live_ids = []
        if collection_type in ["owned", "unowned", "all"]:
        ...
        cursor = (
            self.history_col.find(query).sort(sort_stage).skip(skip).limit(limit)
        )
        results = [doc async for doc in cursor]
        ...
        if collection_type == "owned":
            ...
            for r in results:
                r["is_owned"] = r.get("live_id") in watched_set
    ...
    async def get_total_pc_collection_count(
        ...
        if collection_type in ["owned", "unowned"]:
            watched_live_ids = await self.watched_col.distinct(
                "live_id", {"user_id": user_id}
            )
            if collection_type == "owned":
