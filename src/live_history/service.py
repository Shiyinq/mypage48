...
class LiveHistoryService:
    ...
    async def update_watch_duration(self, user_id: str, data: LiveHistoryUpdateRequest) -> bool:
    ...
    async def get_user_history(
        self, user_id: str, page: int = 1, limit: int = 20, member_id: Optional[str] = None
    ) -> Dict[str, Any]:
        skip = (page - 1) * limit
        
        if member_id:
            docs = await self.repository.get_history_by_user_and_member(user_id, member_id, skip, limit)
        else:
    ...
    async def get_member_stats(self, user_id: str, member_id: str) -> MemberLiveHistoryStatsResponse:
