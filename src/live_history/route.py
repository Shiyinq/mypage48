...
@router.get("", response_model=LiveHistoryPaginationResponse)
...
@router.get("/stats", response_model=LiveHistoryStatsResponse)
...
@router.get("/members/{member_id}/stats", response_model=MemberLiveHistoryStatsResponse)
async def get_member_stats(
    member_id: str,
    current_user: UserCurrent = Depends(get_current_user),
    service: LiveHistoryService = Depends(get_live_history_service)
):
    return await service.get_member_stats(user_id=current_user.userId, member_id=member_id)
